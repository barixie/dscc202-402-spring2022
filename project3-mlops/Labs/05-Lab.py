...
rf2_path = "runs:/"+runID+"/random-forest-model-preprocess/"
...
class RF_with_preprocess(mlflow.pyfunc.PythonModel):
    ...
    def preprocess_input(self, model_input):
        ...
        model_input_copy["trunc_lat"] = model_input_copy["latitude"].round(2)
        model_input_copy["trunc_long"] = model_input_copy["longitude"].round(2)
        model_input_copy["review_scores_sum"] = model_input_copy[['review_scores_accuracy', 'review_scores_cleanliness', 'review_scores_checkin',
                                                                    'review_scores_communication', 'review_scores_location','review_scores_value' ]].sum(axis=1)
        model_input_copy = model_input_copy.drop(["latitude","longitude"], axis=1)
